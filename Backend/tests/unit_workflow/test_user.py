...
def test_register_secu_wrong_key():
...
def test_login_func(login_temp_account):
    ...
    assert data
    LOG.debug(log_debug("login_func", "post", "login", f"payload & access_token = {data}"))

def test_login_secu_wrong_password(login_temp_account):
    exist_user_data = login_temp_account
    username = exist_user_data["username"]
    password = uuid.uuid4().hex
    LOG.debug(log_debug("login_secu_wrong_password", "post", "login", f"payload = username:{username}, password:{password}"))
...
def test_login_secu_wrong_username(login_temp_account):
    ...
    username = uuid.uuid4().hex
    password = exist_user_data["password"]
    LOG.debug(log_debug("login_secu_wrong_username", "post", "login", f"payload = username:{username}, password:{password}"))
    response = User().login(APP_URL, username, password)
    assert response.status_code != 201
...
def test_login_secu_Nonregister():
    username = uuid.uuid4().hex
    password = uuid.uuid4().hex
    LOG.debug(log_debug("login_secu_Nonregister", "post", "login", f"payload = username:{username}, password:{password}"))
    response = User().login(APP_URL, username, password)
    assert response.status_code != 201
...
def test_logout_func(create_temp_account):
    ...
    LOG.debug(log_debug("logout_func", "post", "logout", f"header = {access_token}"))
    response = User().logout(APP_URL, access_token)
    assert response.ok
    LOG.debug(log_debug("logout_func", "post", "logout", f"result = {response.json()}"))

def test_logout_secu_wrong_token():
    access_token = uuid.uuid4().hex
    LOG.debug(log_debug("logout_secu_wrong_token", "post", "logout", f"header = {access_token}"))
    response = User().logout(APP_URL, access_token)
    assert not response.ok
    LOG.debug(log_debug("logout_secu_wrong_token", "post", "logout", f"result = {response.json()}"))
...
def test_get_users_secu_notAdmin(login_temp_account):
    ...
    LOG.debug(log_debug("get_users_func", "get", "user"))
