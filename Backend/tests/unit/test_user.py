...
def test_register_db_same_account(temp_user_account):
    exist_user_data = temp_user_account
    account = exist_user_data["username"]
    password = exist_user_data["password"]
    key = REGISTER_KEY
    LOG.debug(log_debug("db", "post", "register", f'JSON = username:{account}, password:{password}, register_key:{key}'))
    response = User().register(APP_URL, account, password, key)
    assert not response.ok
    LOG.debug(log_debug("db", "post", "register", f'result={response.json()}'))
